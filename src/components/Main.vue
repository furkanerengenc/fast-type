<template>
  <div class="container jumbotron">
    ...
    <p class="lead">Ne kadar hızlı klavye tıklandığını test et!</p>
    <div>
    ...
    <div v-if="isFinish" class="alert alert-primary">
      <h3>Süre Bitti</h3>
      <p class="display-4">{{dks}} DKS (dakikada ki kelime sayısı)</p>
      <span>Doğruluk Yüzdesi: {{truePercent}} </span> <br>
      <span>Doğru Kelime: {{trueCount}}</span> <br>
      <span>Yanlış Kelime: {{falseCount}}</span>
      <button class="btn btn-success btn-lg" @click="newGame">YENİ OYUN</button>
    </div>
    <div v-else>
    <div  class="card">
        <div class="card-body ">

            <span v-for="(word,key) in words.filter((val,index)=>index<10)" :key="key" v-bind:class="key!=0 || writingWordControl" class="words ml-2">
              {{word}}
            </span>
        </div>
    </div>
      ...
      <div class="card-body bg-secondary">
        ...
        <div class="input-group input-group-lg">
          ...
          <div class="input-group-append">
            <button class="btn btn-outline-primary btn-lg" type="button" disabled>{{timer}} sn.</button>
            <button :disabled="isRunning" class="btn btn-outline-primary btn-lg" type="button" @click="getWords">Yenile</button>
          </div>
        </div>
      </div>
    </div>
